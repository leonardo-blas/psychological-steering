...
def get_dest_db_path(model_name: str, mode: str) -> str:
    ...
    subdir = "binary_choice" if mode == "b" else "statement"
...
def fetch_rows(table: str):
    ...
    if len(rows) != CONFIG["expected_rows"]:
        raise ValueError(f"{table} has {len(rows)} rows, expected {CONFIG['expected_rows']}.")
    return rows


def build_texts(rows, mode: str, tokenizer, instruct: bool):
    pairs = []
    statements = []
    labels = []
    ...
    for sys_text, usr, asst in pairs:
        if instruct:
            chat = [{"role": "system", "content": sys_text}, {"role": "user", "content": usr}]
            ...
            prefix = f"{sys_text} {usr} "
        texts.append(prefix + asst)
...
@torch.no_grad()
def extract_states(texts, tokenizer, model, batch_size, prefix_char_lens, answer_char_lens, mode: str):
    device = model.device
    L = getattr(model.config, "num_hidden_layers", None)
    feats = []

    for i in tqdm(range(0, len(texts), batch_size), desc="Extracting activations", unit="batch", leave=False):
...
def concept_done(dest_db_path: str, table: str) -> bool:
    ...
    with sqlite3.connect(dest_db_path) as conn:
        ...
        return cur.fetchone()[0] == CONFIG["expected_rows"]


def save_to_db(dest_db_path: str, table: str, statements, labels, feats):
    ...
    with sqlite3.connect(dest_db_path) as conn:
        ...
        for s, lbl, feat in zip(statements, labels, feats):
            data.append((s, lbl, json.dumps(feat.tolist())))
        cur.executemany(f"INSERT INTO {table} (statement, label, activations) VALUES (?, ?, ?)", data)
        conn.commit()
...
def main():
    ...
    dest_db_path = get_dest_db_path(args.model, args.mode)
    ...
    for concept in tqdm(concepts, desc="Concepts", unit="concept", leave=True):
        seed_all(CONFIG["seed"])
        table = normalize_table_name(concept)
        if concept_done(dest_db_path, table):
            continue
        rows = fetch_rows(table)
        texts, prefix_lens, answer_lens, statements, labels = build_texts(
            rows, args.mode, tokenizer, args.instruct
        )
        feats = extract_states(texts, tokenizer, model, args.batch_size, prefix_lens, answer_lens, args.mode)
        save_to_db(dest_db_path, table, statements, labels, feats)
