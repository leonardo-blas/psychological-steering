...
def build_conflict_adj(sjt_embs):
    ...
    if n <= 1:
        return [0] * n
    sjt_embs_np = sjt_embs.to(torch.float32).cpu().numpy()
    sims = cosine_similarity(sjt_embs_np)
    all_mask = (1 << n) - 1
    adj = [0] * n
    for i in range(n):
        mask = 0
        for j in range(n):
            if i == j:
                continue
            if float(sims[i, j]) > CONFIG["similarity_threshold"]:
                mask |= 1 << j
        adj[i] = mask & all_mask
    return adj
...
def select_topk_from_mis(rows, sjt_embs, item_emb, k):
    ...
    sjt_embs_np = sjt_embs.to(torch.float32).cpu().numpy()
    item_emb_np = item_emb.to(torch.float32).cpu().numpy()
    anchor_sims = cosine_similarity(sjt_embs_np, item_emb_np).reshape(-1).tolist()
    adj = build_conflict_adj(sjt_embs)
...
def main():
    ...
    with sqlite3.connect(CONFIG["raw_sqlite_path"]) as rc:
        ...
        for table in tables_to_process:
            ...
            items_dict = {}
            for dimension, facet, item, key, sjt in rows:
                sjt = (sjt or "").strip()
                if not sjt:
                    continue
                item_key = (dimension, facet, item, key)
                row = {
                    "dimension": dimension,
                    "facet": facet,
                    "item": item,
