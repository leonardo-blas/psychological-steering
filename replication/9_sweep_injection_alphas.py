...
def load_inventory_dimensions(inventory: str):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        cur.execute(
            f"""
            SELECT DISTINCT {quote_ident("dimension")}
            FROM {quote_ident(inventory)}
            WHERE {quote_ident("dimension")} IS NOT NULL
              AND TRIM({quote_ident("dimension")}) <> ''
            ORDER BY {quote_ident("dimension")}
            """
        )
...
def main():
    ...
    concepts = load_inventory_dimensions(args.inventory)
