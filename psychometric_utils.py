...
def load_inventory_rows(table_name, concepts=None, facet=False):
    col = "facet" if facet else "dimension"
    with sqlite3.connect("data/inventories.db") as con:
        cur = con.cursor()
        if concepts:
            if isinstance(concepts, str):
...
def load_sjts_rows(table_name, concepts, facet=False):
    if concepts is None:
        return []
    if isinstance(concepts, str):
        concepts_list = [concepts]
...
def run_inventory(
    ...
    **generate_kwargs,
    ...
    rows = load_inventory_rows(inventory, norm_evaluation_concepts, facet=facet)
    dimensions = []
    statements = []
    keys = []
...
def run_sjts(
    ...
    **generate_kwargs,
    ...
    norm_concepts = _normalize_concepts(concepts)
    questions = load_sjts_rows(inventory, norm_concepts, facet=facet)
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
    tokenizer.padding_side = "left"
    if model_name is None:
